...
export default function Mail() {
	...
	const theme = {
		...
		sidebarBg: darkMode ? 'bg-[#1C1C1E]' : 'bg-[#F2F2F7]',
		headerBg: darkMode ? 'bg-[#1C1C1E]' : 'bg-[#F2F2F7]',
		text: darkMode ? 'text-white' : 'text-black',
		textSecondary: darkMode ? 'text-[#8E8E93]' : 'text-[#8E8E93]',
		borderColor: darkMode ? 'border-[#38383A]' : 'border-[#C6C6C8]',
	...
	return (
		<div className={`w-full h-full flex overflow-hidden font-sans ${theme.bg} ${theme.text}`}>
			...
			>
				...
				<div className={`flex flex-col border-b pt-6 md:pt-0 ${theme.borderColor}`}>
					<div className="h-[44px] flex items-center justify-between px-5">
						<button onClick={toggleEditMode} className="text-[#007AFF] text-[17px]">
							{isEditing ? 'Done' : 'Edit'}
						</button>
						<h1 className="text-[17px] font-semibold">Mailboxes</h1>
				...
				<div className="flex-1 overflow-y-auto">
					<div
						className={`my-4 mx-5 rounded-[10px] overflow-hidden ${darkMode ? 'bg-[#2C2C2E]' : 'bg-white'}`}
					>
						{mailboxesWithCounts
					...
					{mailboxesWithCounts
						...
						.map(account => (
							<div key={account.id} className="mb-2">
								...
								>
									<span
										className={`text-[13px] font-semibold uppercase flex-1 ${theme.textSecondary}`}
									>
								...
								{expandedAccounts.has(account.id) && (
									<div
										className={`mx-5 rounded-[10px] overflow-hidden ${darkMode ? 'bg-[#2C2C2E]' : 'bg-white'}`}
									>
				...
				>
					...
					<div className="text-[11px] text-center">Updated Just Now</div>
					<button
						onClick={() => setIsComposeOpen(true)}
						className="w-10 h-10 flex items-center justify-center text-[#007AFF] active:opacity-50 transition-opacity"
					>
			...
			>
				...
				>
					<div className="h-[44px] flex items-center justify-between px-3">
						...
						<button
							onClick={handleBack}
							className="flex items-center text-[#007AFF] text-[17px] md:hidden"
						>
					...
					<div className="px-5 pb-2">
						<h1 className="text-[34px] font-bold leading-tight capitalize">
							{selectedMailbox.replace('_', ' ')}
						</h1>
					</div>
					<div className="px-5 pb-3">
						<div
							className={`relative flex items-center rounded-[10px] px-3 py-2 ${darkMode ? 'bg-[#2C2C2E]' : 'bg-[#E3E3E8]'}`}
						>
				...
				<div className="flex-1 overflow-y-auto">
					{displayEmails.length === 0 ? (
						<div className="flex flex-col items-center justify-center h-full text-gray-400">
							<p className="text-[17px]">{isUnreadFilterActive ? 'No Unread Mail' : 'No Mail'}</p>
						</div>
					) : (
					...
					{isEditing ? (
						...
						<>
							<button
								...
								className={`w-10 h-10 flex items-center justify-center rounded-full active:opacity-50 transition-opacity ${selectedForAction.size > 0 ? 'text-[#007AFF]' : 'text-gray-400'}`}
							>
								<i className="fas fa-archive text-lg"></i>
							</button>
							<div className={`text-[12px] font-medium ${darkMode ? 'text-white' : 'text-black'}`}>
						...
						<>
							<button
								...
								className={`w-10 h-10 flex items-center justify-center rounded-full active:opacity-50 transition-opacity ${isUnreadFilterActive ? 'bg-[#007AFF] text-white' : 'text-[#007AFF]'}`}
							>
								<i
			...
			>
				...
				>
					<div className="h-[44px] flex items-center justify-between px-3">
						<button onClick={handleBack} className="flex items-center text-[#007AFF] text-[17px]">
							<i className="fas fa-chevron-left text-xl mr-1"></i>
				...
				{selectedEmail ? (
					...
					<div className="flex-1 flex items-center justify-center text-[#8E8E93]">
						<div className="text-center">
							...
							<p className="text-xl font-medium">No Message Selected</p>
						</div>
					</div>
				)}
			</div>
